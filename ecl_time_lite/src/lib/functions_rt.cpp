...
TimeError epoch_time(TimeStructure &time, bool monotonic)
{
  ...
  switch (result)
  {
    ...
    case (EFAULT):
    {
      return TimeError(MemoryError);
    } // time was not in addressable memory space
    case (EINVAL):
    {
      return TimeError(ArgNotSupportedError);
    } // clock id is not supported (actually impossible if cmake detects)
    case (EPERM):
    {
      return TimeError(PermissionsError);
    } // user does not have permissions to use the clock.
    default:
    {
...
TimeError sleep_until(const TimeStructure &time)
{
  ...
  switch (result)
  {
    ...
    case (EINTR):
    {
      return TimeError(InterruptedError);
    } // interrupted by a signal
    case (EINVAL):
    {
      return TimeError(OutOfRangeError);
    } // sec/nsec pair specified was out of range
    default:
    {
      return TimeError(UnknownError);
    }
...
TimeError sleep(const TimeStructure &time)
{
  ...
  switch (result)
  {
    ...
    {
      return TimeError(MemoryError);
    } // some memory error copying information around
    case (EINTR):
    {
      return TimeError(InterruptedError);
    } // interrupted by a signal
