 ...
 */
class ECL_PUBLIC SocketError : public Error {
public:
	/**
...
protected:
	virtual const char* invalidArgErrorString() const { return "One of the arguments is invalid (usually a socket descriptor)."; }
#ifdef ECL_IS_WIN32
	virtual const char* notSupportedError() const { return "This version of winsock is not supported on this platform."; }
	virtual const char* interruptedErrorString() const { return "Interrupted by WSACancelBlockingCall."; }
	virtual const char* notInitialisedErrorString() const { return "The underlying winsock subsystem is not initialised (needs WSAStartup)."; }
	virtual const char* blockingErrorString() const { return "Marked as non-blocking, but current configuration would block."; }
	virtual const char* busyErrorString() const { return "A blocking winsock operation is in progress."; }
	virtual const char* systemFailureErrorString() const { return "The network subsystem has failed."; }

#elif defined(ECL_IS_POSIX)
	virtual const char* interruptedErrorString() const { return "Interrupted by a signal or an io error."; }
	virtual const char* argNotSupportedString() const { return "The specified address family or protocol is not supported on this machine."; }
#endif
};
